...
class SpatialPooler(object):
  ...
  def compute(self, inputVector, learn, activeArray):
    ...
    if hasattr(self, '_topologicalInhibition') and self._topologicalInhibition:
      self._boostedOverlaps = self._inhibitColumnsMirko(self._boostedOverlaps)
  ...
  def _updatePairwiseActivity(self, activeColumns):
    ...
    for j in range(self._numColumns):
      self._pairwiseActivity[j,j] = self._updateDutyCyclesHelper(
        self._pairwiseActivity[j,j], activeArray[j], period)
  ...
  def _updateInhibitionGraph(self):
    ...
    for i in range(self._numColumns):
      for j in range(self._numColumns):
        ...
        if i != j:
          a_ij = self._pairwiseActivity[i][j]
          a_j  = self._pairwiseActivity[j][j]

          if a_ij/a_j >  1.5*density:
            self._inhibitionGraph[i].add(j)

          elif a_ij/a_j < 0.5*density:  
  ...
  def _inhibitColumnsMirko(self, overlaps):
    """
    Indirectly performs inhibition by applying a 2nd boosting round based on the inhibition graph.
    ...
    doubleboostedOverlaps = numpy.copy(overlaps)
    ...
    # overlap defines orientation on the edges of the inhibition graph
    # each parent i "boosts" its children j by a factor computed from the 
    # pairwise activity aij with target activity ai
    edges = []
    bigger = set([])
    for i in sortedByOverlap:
      children = self._inhibitionGraph[i];
      bigger.add(i)
      for j in children:
        if j not in bigger:
          edges.append([i,j])
    ...
    for i,j in edges:
      a_ij = self._pairwiseActivity[i][j]
      a_j  = self._pairwiseActivity[j][j] 
      doubleboostedOverlaps *= numpy.exp(-(a_ij - a_j) * 10)

    return doubleboostedOverlaps
